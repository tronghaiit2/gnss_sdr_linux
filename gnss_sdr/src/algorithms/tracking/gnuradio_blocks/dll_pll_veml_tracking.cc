      ...
      d_Flag_PLL_180_deg_phase_locked(false)
{
    ...
    //haint
    // sendData = SendData();
    // char file[sendData.BUFSIZE];
    // file = "/tmp/tracking.txt";
    char* file = (char*) malloc(sizeof(char) * sendData.BUFSIZE);
    snprintf(file, sendData.BUFSIZE, "/tmp/tracking.txt");
    sendData.init(file);
    free(file);
    sample_count = 0;
    //haint
...
void dll_pll_veml_tracking::start_tracking()
{
    ...
    //haint
    // char* str;
    // sprintf(str, "Tracking of %s %s signal started on channel %d for satellite %s", d_systemName, d_signal_pretty_name, d_channel, Gnss_Satellite(d_systemName, d_acquisition_gnss_synchro->PRN));
    // sendData.sendData(str);

    // char *str = (char*) malloc(sizeof(char) * sendData.MSIZE);
    // Gnss_Satellite gnss_Satellite = Gnss_Satellite(d_systemName, d_acquisition_gnss_synchro->PRN);
    // snprintf(str, sendData.MSIZE, "Tracking of %s %s signal started on channel %d for satellite %d (%s)", d_systemName.c_str(), d_signal_pretty_name.c_str(), d_channel, gnss_Satellite.get_PRN(), gnss_Satellite.get_block().c_str());
    // sendData.sendData(str);
    // free(str);
    //haint
...
bool dll_pll_veml_tracking::cn0_and_tracking_lock_status(double coh_integration_time_s)
{
    ...
    if (d_carrier_lock_fail_counter > d_trk_parameters.max_carrier_lock_fail or d_code_lock_fail_counter > d_trk_parameters.max_code_lock_fail)
        ...
        // Make array to calcualte NBP and WBP
        if(sample_count < 20) {
            std::map<int, std::vector<S4>>::iterator it;
            int prn = d_acquisition_gnss_synchro->PRN;
            // channel I
            it = S4_map.find(prn); 
            S4 s4 = {prompt_I, prompt_Q};
            if (it == S4_map.end()) {
                S4_map.insert(std::make_pair(prn, std::vector<S4>{s4}));
            }
            else {
                it->second.push_back(s4);
            }
            sample_count++;
        } else {
            for(auto it = S4_map.cbegin(); it != S4_map.cend(); ++it)
            {
                // if(it->first == 5) {
                char *str = (char*) malloc(sizeof(char) * sendData.MSIZE);
                // std::cout << "PRN: " << d_acquisition_gnss_synchro->PRN << " CNO: " << d_CN0_SNV_dB_Hz << "\n";
                // Gnss_Satellite gnss_Satellite = Gnss_Satellite(d_systemName, d_acquisition_gnss_synchro->PRN);
                // snprintf(str, sendData.MSIZE, "{\"PRN\": %d, \"CNO\": %.5f, \"PromptI\": %.5f, \"PromptQ\": %.5f}", d_acquisition_gnss_synchro->PRN, d_CN0_SNV_dB_Hz, prompt_I, prompt_Q);
                double SIraw = dll_pll_veml_tracking::SIraw(it->second);
                snprintf(str, sendData.MSIZE, "{\"PRN\": %d, \"CNO\": %.5f, \"SIraw\": %.5lf}", it->first, d_CN0_SNV_dB_Hz, SIraw);
                // snprintf(str, sendData.MSIZE, "{\"PRN\": %d, \"CNO\": %.5f}", d_acquisition_gnss_synchro->PRN, d_CN0_SNV_dB_Hz);
                if(str != nullptr) sendData.sendData(str);
                free(str);
                // }
            }
            sample_count = 0;
...
double dll_pll_veml_tracking::SIraw(std::vector<S4> const& S4_list) {
    double sumI = 0.0, sumQ = 0.0, sumIQ = 0.0;
    for (auto it = S4_list.begin(); it != S4_list.end(); it++) {
        sumI += it->channel_I;
        sumQ += it->channel_Q;
        sumIQ += it->channel_I*it->channel_I + it->channel_Q*it->channel_Q;
    }
    return sumI*sumI + sumQ*sumQ - sumIQ;
}
                                                ...
                                                if (next_state)
                                                    {
                                                        ...
                                                        //haint
                                                        // char* str;
                                                        // sprintf(str, "%s %s tracking bit synchronization locked in channel %d for satellite GPS PRN 20 (Block IIR)", d_systemName, d_signal_pretty_name, d_channel, Gnss_Satellite(d_systemName, d_acquisition_gnss_synchro->PRN));
                                                        // sendData.sendData(str);

                                                        // char *str = (char*) malloc(sizeof(char) * sendData.MSIZE);
                                                        // Gnss_Satellite gnss_Satellite = Gnss_Satellite(d_systemName, d_acquisition_gnss_synchro->PRN);
                                                        // snprintf(str, sendData.MSIZE, "%s %s tracking bit synchronization locked in channel %d for satellite %d (%s)", d_systemName.c_str(), d_signal_pretty_name.c_str(), d_channel, gnss_Satellite.get_PRN(), gnss_Satellite.get_block().c_str());
                                                        // sendData.sendData(str);
                                                        // free(str);
                                                        //haint
